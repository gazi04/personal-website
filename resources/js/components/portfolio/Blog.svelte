    ...
    <div class="flex flex-col">
      {#each blogPosts as post, i}
        <a href={`/blog/${post.slug}`}
          ...
          <span class="font-['JetBrains_Mono'] text-[0.7rem] text-[#cbd5e1] tracking-widest max-sm:hidden">
            0{i + 1}
          </span>

          <div class="flex flex-col gap-1.5">
            <h3 class="font-['DM_Sans'] text-[1.1rem] font-semibold text-[#0f172a] leading-tight">{post.title}</h3>
            <p class="font-['Inter'] text-[0.85rem] text-[#64748b] leading-relaxed">{post.summary}</p>
          </div>

          <div class="flex flex-col items-end gap-1 font-['JetBrains_Mono'] text-xs text-[#94a3b8] whitespace-nowrap">
            <span>{post.readTime}</span>
            <span>{post.date}</span>
          </div>

          <span class="text-[#cbd5e1] text-base transition-all duration-150 max-sm:hidden group-hover:translate-x-1 group-hover:text-[#0f172a]">
            →
          </span>
